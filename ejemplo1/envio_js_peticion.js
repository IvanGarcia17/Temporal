...
//uso el llamado get con parámetros en json
...
btnGetJson.addEventListener("click", function() {
    ...
    axios.get("http://localhost4567/holaJson?" + params)
        .then(function (rs) {
            console.log(rs.data);
            //alert(rs.data.time)
        })
...
btnPostJson.document.addEventListener("click", function () {
    var params = new URLSearchParams();
    var email = document.getElementById("exampleInputEmail1").value;
    var pass = document.getElementById("exampleInputPassword1").value;
    params.append("nombre", email);
    ...
    console.log("------")
    console.log("{" + "name: '" + params.get("nombre") + "'," + "pass: '" + params.get("pass") + "'}");
    var params2 = "{" + "name: '" + params.get("nombre") + "'," + "pass: '" + params.get("pass") + "'}"; //forma equívoca de construir el objeto, ya que le sobran las llaves
